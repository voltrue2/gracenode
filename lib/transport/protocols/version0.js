...
function parse(buf) {
	...
	var plen = buf.readUInt32BE(OFFSET_PLEN) & 0x00ffffff;
	if (plen >= MAX_PAYLOAD_SIZE) {
		// disconnect please
		return new Error(ERR_PAYLOAD_MAX);
	}
	// calculate packet size
	var packetLen = plen + SIZE_HEADSTOP_SIZE;
	if (bufLen < packetLen) {
		// incomplete packet, wait for more to come
		return null;
	}
...
function parsePayload(buf, plen) {
	...
	var mStopPos = plen + SIZE_HEADER_SIZE;
	var mBytes = buf.slice(mStopPos, mStopPos + SIZE_STOP_SIZE);
