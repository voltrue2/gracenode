...
const PAYLOAD_HEADER_SIZE = PAYLOAD_SIZE_SIZE + PAYLOAD_CMD_SIZE + PAYLOAD_SEQ_SIZE;
...
function packPayloads(payloads) {
	...
	for (var i = 0, len = payloads.length; i < len; i++) {
		var item = payloads[i];
		const size = PAYLOAD_HEADER_SIZE + item.payload.length;
	...
	packs.push(MSTOP);
	// combine all packets and stop symbol bytes
