...
module.exports.createPush = function (seq, payload) {
	// TODO: add different protocol versions
	logger.verbose('<push> protocaol version:', 0, 'push packet:', payload);
...
function parseV0(buf) {
	...
	logger.verbose('parse', protoName, 'packet: (size:' + bufLen + ')');
	...
	logger.verbose('parse', protoName, 'payload size:', plen);
...
function parsePayload(version, type, mStopPos, buf, payload) {
	...
	if (buf.readUInt8(offset.REPLY_FLAG) === 0x01) {
		...
		// reply packet cannot have command
		delete packet.command;
	}
...
Stream.prototype.parse = function (buf) {
	if (!buf) {
...
Stream.prototype._parse = function () {
	logger.verbose('parsing buffer stream:', this.buffer.length);
