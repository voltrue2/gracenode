...
function whilstParallel(max, each, cb) {
    if (!max) {
        return cb();
    }
    var baggage = { counter: 0, length: max };
    var bind = { baggage: baggage, cb: cb };
    for (var i = 0; i < max; i++) {
        each(_onParallelEach.bind(null, bind));
    }
}

function whilst(max, each, cb) {
    if (!max) {
        return _finish(cb);
    }
    var bind = { max: max - 1, each: each, cb: cb };
    each(_onWhilst.bind(null, bind));
}

function _onWhilst(bind, error) {
    var max = bind.max;
    var each = bind.each;
    var cb = bind.cb;
    if (error) {
