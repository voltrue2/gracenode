...
function ctr(cipher, cipherNonce, seq, input) {
	...
	for (var blockNum = 0; blockNum < totalBlocks; blockNum++) {
		...
		// xor chunk (end - start = BLOCK_LEN or less) and cipher.update(counter)
		xor(input.slice(start, end), cipher.update(counter), start, output);
	}
	return output;
}

// we hard-code this instead of loop b/c we know the max length of chunk is BLOCK_LEN
function xor(chunk, counterCrypt, offset, output) {
	var counter = 0;
	const len = chunk.length;
	if (counter < len) {
		output[counter + offset] = chunk[counter] ^ counterCrypt[counter];
		counter += 1;
	}
	if (counter < len) {
		output[counter + offset] = chunk[counter] ^ counterCrypt[counter];
