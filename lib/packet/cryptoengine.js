...
CryptoEngine.prototype.decrypt = function (cipherKeyIn, cipherNonceIn, macKeyIn, seq, payload) {
...
*/
CryptoEngine.prototype.encrypt = function (cipherKeyIn, cipherNonceIn, macKeyIn, useq, payload) {
	var cipherKey = toBuffer(cipherKeyIn);
	var cipherNonce = toBuffer(cipherNonceIn);
	var macKey = toBuffer(macKeyIn);
	var cipher = createCipher(cipherKey);
...
function createCipher(cipherKey) {
	var cipher = crypto.createCipheriv('aes-128-ecb', cipherKey, new Buffer(0));
	return cipher;
}
