...
function CryptoEngine() {
	//TODO: session ID + seq must be checked
	// seq w/ the same session ID that is smaller than what the server has must be discarded
}
...
// static function
CryptoEngine.createCipher = function () {
	return {
		cipherKey: crypto.randomBytes(16),
		cipherNounce: crypto.randomBytes(8),
...
/*
TODO: also aes-128-ecb should not be used
if we are encrypting more than one block of data consider using cbc
but we are also using CTR w/ random IV/nounce... hmm safe enough right?
*/
