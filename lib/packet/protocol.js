...
function parseDataRPC(inputBuff) {
	...
	// bitwise operation &
	var payloadLen = inputBuff.readUInt32BE(OFFSET.PLEN) & 0x00ffffff;
	...
	if (inputBuff.length >= packetLen) {
		var magicStopPos = OFFSET.PLEN + payloadLen + V0_PACKET_HEADER_SIZE;
		...
		if (magicVal === MAGIC_STOP_SYMBOL) {
			...
			var sec = inputBuff.readUInt32BE(OFFSET.TIMESTAMP);
