...
module.exports.setup = function (gn) {
	...
	logger.info('maximum packet payload size is:', MAX_PAYLOAD_SIZE, 'bytes');
};
...
function parseData(inputBuff) {
	...
	logger.verbose('protocol version:', protocolVer);
...
function parseDataRPC(inputBuff) {
	...
	logger.verbose('parse packet as RPC: packet size', inputBuff.length);
	...
	// byte offset for payload size is at 1 byte
	var payloadlenOffset = 0;

	// bitwise operation &
	var payloadLen = inputBuff.readUInt32BE(payloadlenOffset) & 0x00ffffff;

	
	logger.verbose('payload size:', payloadLen);
	...
	if (inputBuff.length >= packetLen) {
		...
		logger.verbose(magicVal, 'must be same as', MAGIC_STOP_SYMBOL);

		// disconnect now
		throw new Error(ERR.BAD_MAGIC_SYMBOL);
	}

	logger.verbose('incomplete packet: must be', inputBuff.length, '>=', packetLen);
