...
module.exports.setup = function () {
	logger = gn.log.create('RPC.connHandler');
};

module.exports.create = function (sock, options) {
	return new ConnectionHandler(sock, options);	
};

function ConnectionHandler(sock, options) {
...
utils.inherits(ConnectionHandler, EventEmitter);

ConnectionHandler.prototype.useCryptoEngine = function (engine) {
	this.crypt = engine;
};

ConnectionHandler.prototype.close = function (error) {
	if (error) {
...
ConnectionHandler.prototype.kill = function (error) {
	if (this.sock) {
		logger.info(this.name, 'TCP connection killed from server:', error);
		this.sock.destroy();
	}
	this._clear(true);
};

ConnectionHandler.prototype._data = function (packet) {
	var that = this;
...
ConnectionHandler.prototype._decrypt = function (parsedData, cb) {
...
ConnectionHandler.prototype._routeAndExec = function (parsedData, sess, cb) {
...
ConnectionHandler.prototype._errorResponse = function (parsedData, sess, cb) {
...
ConnectionHandler.prototype._execCmd = function (cmd, parsedData, sess, cb) {
	var that = this;
	var state = createState(this.id, parsedData, sess);
...
ConnectionHandler.prototype._write = function (_error, state, status, seq, msg, cb) {
	var that = this;
...
ConnectionHandler.prototype._push = function (state, msg, cb) {
	var that = this;
	if (typeof msg === 'object' && !(msg instanceof Buffer)) {
		msg = JSON.stringify(msg);
	}
...
ConnectionHandler.prototype.__write = function (error, data, cb) {
...
ConnectionHandler.prototype.__push = function (data, cb) {
	if (!this.sock || !this.connected) {
		if (!cb) {
			return;
		}
...
ConnectionHandler.prototype._encrypt = function (state, msg, cb) {
...
ConnectionHandler.prototype._clear = function (killed) {
