...
var MAGIC_STOP_SYMBOL = 0xdeadbeef;
...
function parseData(inputBuff) {
	...
	switch (protocolVer) {
		case 0:
			return parseDataVersion0(inputBuff);
		case 0x50:
...
function parseDataProxyVersion1(inputBuff) {
	...
	if (inputBuff.length > MAX_PROXY_V1_LEN) {
		...
		throw new Error(ERR.UNKNOW_PV1);
	}
	// we don't seem to have enough data yet
	return null;
}

function parseDataVersion0(inputBuff) {
	...
	if (inputBuff.length >= packetLen) {
		...
		if (magic.readUInt32BE(0) === MAGIC_STOP_SYMBOL) {
			...
			packet.seq = inputBuff.readUInt16E(6);
			packet.payload = new Buffer(payloadLen);
			inputBuff.copy(packet.payload, 0, V0_PACKET_HEADER_SIZE, magicStopPos);
			packet.consumedLen = packetLen;
			return packet;
		}
		// disconnect now
		throw new Error(ERR.BAD_MAGIC_STOP_SYMBOL);
	}
