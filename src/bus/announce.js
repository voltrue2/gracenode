...
module.exports.config = function (_conf) {
	...
	logger = gn.log.create('bus.announce');
};
...
module.exports.setup = function (cb) {
	...
	gn.onExit(function busAnnounceShutdown(next) {
		closeConnection();
		next();
	});
};
...
function startAnnounceAndRead() {
	...
	const announce = function __busAnnounce(next) {
		if (!rclient) {
			return done();
		}
		...
		multi.exec(function __busOnAnnounce(error) {
			if (error) {
	...
	const read = function __busRead(next) {
		if (!rclient) {
			return done();
		}
		scan(function __busOnRead(error) {
			if (error) {
...
function scan(cb) {
	...
	var list = [];
	const multi = rclient.multi();
	const callback = function __busScanCallback(error, res) {
		if (error) {
			return cb(error);
		}
		...
		multi.exec(function __busOnScanGet(error, results) {
			if (error) {
				return cb(error);
			}
	...
	const scanner = function __busScanner() {
		if (!rclient) {
			return cb(new Error('RedisIsOffline')); 
		}
		rclient.scan(
