...
const DSCHEMA = '__bus__delivery__schema';
...
module.exports.config = function (_conf) {
	...
	logger = gn.log.create('bus.delivery');
};
...
module.exports.schema = function (name, struct) {
	if (namelist.indexOf(name) !== -1) {
		throw new Error('BusDeliveryShcemaNameAlreadyUsed');
	}
	namelist.push(name);
...
module.exports.send = function (name, deliveryData, destlist, cb) {
	...
	if (id === -1) {
		cb(new Error('InvalidBusDeliveryName:' + name));
	}
...
function sendDelivery(nexts, addr, port, data, cb) {
	...
	if (!addr || !port) {
		const err = new Error('BusDeliveryError');
		logger.error(
...
function startServer(cb) {
	const done = function () {
		gn.onExit(function busDeliveryShutdown(next) {
			try {
				server.close();
				server = null;
			} catch (error) {
