...
#### UDP server performance improvements by removing the system call of gettimeofday on every packet received
...
#### gracenode core process stop logging changed from info to verbose (stack trace part only)
...
#### RPC server added .requireCallback(<timeoutInMilliseconds>)
...
#### RPC removed pushInterval

The configuration for RPC module has been removed as it puts more stress on performance.
...
#### Transport bin(binary) Stream is now asynchronous
...
#### gracenode.lib.now() added

This method is to reduce the number of syscall `gettimeofday` when calling `Date.now()` in exchange of accuracy.

## Changed

#### RPC max packet size now supports transport packet size also
...
#### RPC server improved connection management and graceful shutdown
...
#### RPC state object added .close() and .kill()

#### RPC connection object performance improvements: removed a global event emitter
...
#### RPC socket write corrected encoding
